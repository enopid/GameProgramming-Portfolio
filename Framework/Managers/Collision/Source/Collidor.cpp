...
HRESULT CCollidor::SetBakedSetting(_uint idx)
{
    ...
    if (auto _spModel = m_wpModel.lock()) {
        if (_spModel->Get_NumMeshes() > (Engine::_uint)m_desc.m_collidorDesc[idx].m_iMeshIdx && m_desc.m_collidorDesc[idx].m_iMeshIdx > 0) {
...
void CCollidor::Update(float fDeltaTime)
{
    ...
    for (size_t i = 0; i < 3; i++)
    {
        if (m_pBounding[i]) {
            ...
            if (in(m_desc.m_iCollidorIdx, { 1,2,3,4 }))     m_pBounding[i]->SetColor(Colors::Tomato);
            if (in(m_desc.m_iCollidorIdx, { 5,6,7 }))       m_pBounding[i]->SetColor(Colors::Red);
            if (in(m_desc.m_iCollidorIdx, { 8,9,10 }))      m_pBounding[i]->SetColor(Colors::Yellow);
            if (in(m_desc.m_iCollidorIdx, { 11,12,13 }))    m_pBounding[i]->SetColor(Colors::LightYellow);
            if (in(m_desc.m_iCollidorIdx, { 14,15 }))       m_pBounding[i]->SetColor(Colors::LightBlue);
...
HRESULT CCollidor::Late_Initialize()
{
...
HRESULT CCollidor::SetCollidor(const size_t& i)
{
    ...
    case Engine::EBOUNDINGSOURCE::CUSTOM:
        CHKFAIL(SetCustomSetting((Engine::_uint)i))
            break;
    case Engine::EBOUNDINGSOURCE::BONEDRIVEN:
        CHKFAIL(SetBoneDrivenSetting((Engine::_uint)i))
        break;
    case Engine::EBOUNDINGSOURCE::BAKED:
        CHKFAIL(SetBakedSetting((Engine::_uint)i))
            break;
    default:
        break;
    }
...
void CCollidor::Render_SingleCollidor(_uint idx)
{
    ...
    if      (m_desc.m_collidorDesc[idx].m_eBoundingSrcType == ENUM_TO_UINT(EBOUNDINGSOURCE::BONEDRIVEN)) {
        ...
        if (m_wpModel.lock()->Get_NumMeshes() > 0)
            if (ImGui::DragInt("MeshIdx", &m_desc.m_collidorDesc[idx].m_iMeshIdx, 1.f, 0, m_wpModel.lock()->Get_NumMeshes())) {
                Safe_Release(m_pBounding[idx]);
                m_desc.m_collidorDesc[idx].m_boundingInfo.m_CapsuleDesc = CBounding_Capsule::CAPSULE_DESC{};
                SetCollidor(idx);
            }
