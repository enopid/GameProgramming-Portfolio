...
> 현재 저장소는 공개 범위 검토 단계입니다. 검토가 끝난 뒤 각 항목에 핵심 소스를 추가합니다.
...
- Toodee and Topdee는 위에 명시한 콘텐츠 코드만 공개합니다.
- Hi-Fi RUSH는 명시한 콘텐츠와 직접 작성한 프레임워크 코드를 공개합니다.
- `ThirdParty`, 빌드 산출물, 원작 리소스와 팀원 코드는 제외합니다.
- 전체 프로젝트 의존성을 제거하므로 일부 코드는 이 저장소만으로 실행되지 않을 수 있습니다.

실제 복사 예정 파일과 확인 사항은 [공개 코드 선정표](docs/CODE_SELECTION.md)에 정리했습니다.
