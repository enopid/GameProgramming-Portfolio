...
HRESULT CAnimNode::LoadClip(ANIMCLIP_DESC& _desc, Json::Value jsonValue)
{
    ...
    if (jsonValue.isMember("RootMotionScale")) {
        _desc.fRootMotionScale.x = jsonValue["RootMotionScale"][0].asDouble();
        _desc.fRootMotionScale.y = jsonValue["RootMotionScale"][1].asDouble();
    }
    if (jsonValue.isMember("ForceReset")) {
        _desc.bIsForceReset = true;
    }
    if (jsonValue.isMember("Speed")) {
        _desc.fAnimationSpeed = jsonValue["Speed"].asDouble();
    }
    return S_OK;
}
...
unique_ptr<CAnimNode> CAnimNode::Create(Json::Value jsonValue, shared_ptr<CModel> spModel)
{
    ...
    else if (_sType == "ACTION") {
        upAnimNode = move(CActionAnimNode::Create(jsonValue, spModel));
    }
    else if (_sType == "LAYER") {
        auto upLayerNode = CLayerAnimNode::Create(jsonValue, spModel);
        upAnimNode = move(upLayerNode);
    }
    else if (_sType == "LAYERACTION") {
        auto upLayerActionNode = CLayerActionAnimNode::Create(jsonValue, spModel);
        upAnimNode = move(upLayerActionNode);
    }
    else {
