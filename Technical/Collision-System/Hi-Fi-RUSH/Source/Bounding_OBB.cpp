...
CONTACTMF_DESC CBounding_OBB::ComputeManifold(CBounding_OBB* pTarget)
{
	...
	fPenetration	+= 2.f * XMVectorGetX(XMVector3Length(XMLoadFloat3(&m_pDesc->Extents)));
	fPenetration	+= 2.f * XMVectorGetX(XMVector3Length(XMLoadFloat3(&_pTargetDesc->Extents)));
	fPenetration	+= 1.f * XMVectorGetX(XMVector3Length(_vCenterDist));
